...
| Project coordinates
| `property` | OPTION
| Definition of property <emphasis>key</emphasis> with value _value_
| `provided` | GAV[;OPTION]*
...
This Feature is deprecated and superceeded by the Eclipse Classpath Container for Cmvn (see <xref linkend="ch:eclipseCpContainer" />).
...
Notice: If you use the Eclipse Cmvn classpath container, you usually do not want to use the `eclipseClasspath autoGenerate` setting (see <xref linkend="sec:settingsEclipseClasspath" />).
